...
*使用 cat 命令查看 $SYSTEM_CONFIG_DIR/os-release 文件*

```
cat $SYSTEM_CONFIG_DIR/os-release
```

*使用 cat 命令查看 $SYSTEM_CONFIG_DIR/issue 文件*

```
cat $SYSTEM_CONFIG_DIR/issue
```
...
2、如果需要添加或修改语言环境，可以使用编辑器打开 $SYSTEM_CONFIG_DIR/locale.gen 文件;
...
5、最后，在系统环境变量中添加语言选项;
		5.1、可以编辑 $SYSTEM_CONFIG_DIR/environment 文件，在文件末尾添加 LANG=语言选项，例如：LANG=zh_CN.UTF-8。
6、重新启动或注销系统，使语言选项生效;
...
2、输入以下命令以打开locale配置文件：
   nano $SYSTEM_CONFIG_DIR/default/locale
3、确认文件内容如下：
   LANG=en_US.UTF-8
...
4、使用Ctrl + X键保存并关闭文件;
5、输入以下命令以重新加载环境变量：
   source $SYSTEM_CONFIG_DIR/default/locale
6、重启服务器，以确保语言设置成功：
   sudo reboot
```
...
2、编辑 $SYSTEM_CONFIG_DIR/default/locale 文件，添加以下内容：
	LANG="zh_CN.UTF-8"
...
- 通过创建或修改$SYSTEM_CONFIG_DIR/timezone文件来设置时区，例如：
    $ sudo echo "Asia/Shanghai" > $SYSTEM_CONFIG_DIR/timezone
...
```shell
sudo security add-trusted-cert -d -r trustRoot -k $SYSTEM_LIBRARY_DIR/Keychains$SYSTEM_DIR.keychain cert.pem
```
...
```
SSLEngine on
SSLCertificateFile $SYSTEM_CONFIG_DIR/letsencrypt/live/yourdomain.com/fullchain.pem
SSLCertificateKeyFile $SYSTEM_CONFIG_DIR/letsencrypt/live/yourdomain.com/privkey.pem
```
...
***Nginx**的配置文件位于 `$SYSTEM_CONFIG_DIR/nginx/nginx.conf`*

##### 4.2.2.1、首先备份原始配置文件

```
sudo cp $SYSTEM_CONFIG_DIR/nginx/nginx.conf $SYSTEM_CONFIG_DIR/nginx/nginx.conf.original
```
...
error_log $SYSTEM_LOG_DIR/nginx/error.log warn;
pid /run/nginx.pid;
events {
    worker_connections 1024;
}
http {
  include $SYSTEM_CONFIG_DIR/nginx/mime.types;
  ...
  log_format   main '$remote_addr - $remote_user [$time_local]  '
                     ...
                     '"$gzip_ratio"';
  sendfile on;
  access_log $SYSTEM_LOG_DIR/nginx/access.log main;
  keepalive_timeout 65;
  include $SYSTEM_CONFIG_DIR/nginx/conf.d/*.conf;
}
```
...
```
nginx: the configuration file $SYSTEM_CONFIG_DIR/nginx/nginx.conf syntax is ok
nginx: configuration file $SYSTEM_CONFIG_DIR/nginx/nginx.conf test is successful
```
