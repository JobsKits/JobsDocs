...
draft: false
weight: 350
summary: "正则表达式的相关判定 - 1、检查一个手机号是否为中国的手机号 - 2、正则表达式匹配某一段字符串 - 2.1、正则表达式匹配英文（大小写）、空格、任意字符串（例如：Jobs）、数字、中文字符（繁 + 简） - 2.2、正则表达式匹配中文字符 - 2.3、正则表达式匹配 Emoji - 2.4、正则表达式匹配网页地址 - 2.5、正则表达式匹配特殊字符 - "
