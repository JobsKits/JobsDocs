...
* `【MacOS】放在iOS项目工程根目录下，自动打包并输出为ipa文件.command`
  ...
  setup_xcode() {
      local XCODE_DIR="$APPLICATIONS_DIR/Xcode.app"
      if [ ! -d "$XCODE_DIR" ]; then
  ...
  install_package_application() {
      local PACKAGE_DIR="$1/Contents/Developer/Platforms/iPhoneOS.platform/Developer$SYSTEM_USR_DIR/bin"
  ...
  main() {
      ...
      self_intro
      setup_xcode
      install_package_application "$APPLICATIONS_DIR/Xcode.app"
      build_app
