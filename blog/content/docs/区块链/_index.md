...
```bash
open $SYSTEM_DIR/Library/Image\ Capture/Devices/VirtualScanner.app/Contents/Resources/simpledoc.pdf
```
...
## 4、2026 年增量：从“链上账本”到分层基础设施 <a href="#前言摘要" style="font-size:17px; color:green;"><b>🔼</b></a>

> 本章更新时间：`2026-08-12`。前文保留了早期学习脉络，但其中部分观点已经过时或表述过度简化。本章以“纠偏 + 增量”的方式继续往下写；凡涉及尚未落地的升级、监管草案或市场判断，明确标记为计划或观察，不把路线图写成既成事实。

### 4.1、先修正旧笔记里的几个核心认知

| 旧表述或常见误解 | 更准确的理解 |
| --- | --- |
| 以太坊是交易所平台 | 以太坊是可编程区块链网络；交易所只是建立在链上或链外的交易服务 |
| 有 51% 节点认可交易就成功 | “51%”只适用于某些多数攻击讨论，不是所有链逐笔交易的通用投票规则；交易是否有效由状态转换规则决定，最终性由具体共识协议决定 |
| 公钥与私钥可以互相加解密 | 数字签名不是“用私钥加密”；签名、验签、加密、解密是不同密码学原语，许多区块链只用签名而不用公钥加密交易内容 |
| 对称加密安全性低，非对称加密安全性高 | 两者解决的问题不同；在正确算法、密钥长度与协议下都可以安全，实际系统通常组合使用 |
| 区块链写入后绝对不可修改 | 单个区块可能因重组而改变；达到经济或协议最终性后修改成本很高，但治理、合约升级、硬分叉仍能改变后续规则 |
| 链上能直接存任意视频和图像 | 技术上可以写字节，但容量、Gas、长期复制成本极高；大文件通常放链下，只把哈希、承诺或索引放链上 |
| 跨链是侧链的升级版 | 侧链、Rollup、跨链消息和资产桥是不同维度；跨链不会自动继承源链安全性 |
| 所有币都会因丢私钥而越来越少 | 丢失密钥只会让对应资产不可动；有的资产仍会增发，有的可冻结或重发，供应规则取决于协议与发行方 |
| 冷钱包是“不联网的钱包” | 冷钱包的关键是私钥生成和签名环境隔离；链上余额仍记录在网络中，设备本身不保存“币” |

### 4.2、用六层模型重新理解区块链

把“区块链是一种数据库”继续往下拆，可以得到六个层次：

| 层次 | 它回答的问题 | 常见实现 |
| --- | --- | --- |
| 密码学层 | 谁授权了操作、数据是否被篡改 | 哈希、数字签名、Merkle Tree、承诺、零知识证明 |
| 数据可用性层 | 验证者能否取得重建状态所需的数据 | L1 区块数据、Blob、专用 DA 网络 |
| 共识层 | 节点如何同意区块顺序与最终性 | PoW、PoS、BFT 类协议 |
| 执行层 | 一笔交易怎样改变状态 | UTXO、EVM、WASM、并行执行环境 |
| 结算层 | 欺诈或争议最终在哪里裁决 | L1 验证、欺诈证明、有效性证明 |
| 应用层 | 用户实际使用什么 | 钱包、DEX、借贷、稳定币、NFT、身份和供应链应用 |

这套模型比“公链 / 侧链 / 私链”更适合解释今天的模块化架构。一个系统可以让不同网络分别承担执行、数据可用性和结算，而不是所有工作都挤在一条链上。

### 4.3、以太坊的主线已经转为 Rollup-Centric

旧路线常把“执行分片”理解成以太坊扩容的最终答案。当前主线已经转向：L1 负责共识、数据可用性与结算，大量执行发生在 L2 Rollup。

```mermaid
flowchart TB
    U[用户与钱包] --> L2[L2 Rollup 执行与排序]
    L2 --> B[批量交易或状态差异]
    B --> DA[L1 数据可用性]
    B --> S[L1 结算与争议裁决]
    S --> F[最终状态]
...
- Optimistic Rollup 默认接受结果，在挑战期内依赖欺诈证明纠错。
- ZK / Validity Rollup 提交有效性证明，让 L1 验证计算正确性。
- 侧链用自己的验证者和共识，安全边界不同，不能因为“连接以太坊”就等同继承以太坊安全性。
- Validium 把数据放到链下，可降低成本，但增加数据不可用的信任假设。

[**Fusaka**](https://ethereum.org/roadmap/fusaka/) 已于 `2025-12-03` 在以太坊主网上线，其中 PeerDAS 让节点通过采样而不是每个节点下载全部 Blob 数据，为 Rollup 提供更多数据空间。后续 [**Glamsterdam**](https://ethereum.org/roadmap/glamsterdam/) 在本文更新时间仍属于规划中的 `2026` 年升级，具体内容和日期可能变化，不能提前写成“已经完成”。

### 4.4、账户正在从“一个私钥控制一个地址”变成可编程对象

传统外部账户 EOA 的典型问题是：私钥丢失即失控、用户必须持有原生 Gas、授权与执行割裂。

账户抽象不是取消密码学所有权，而是让验证与执行策略可编程：

- 多签、社交恢复和限额。
- Passkey、硬件密钥和多因素验证。
- 批量操作与一次确认完成多步交易。
- 第三方代付 Gas，或使用指定代币支付手续费。
- Session Key，只给某个游戏或应用有限权限和有限期限。

[**ERC-4337**](https://eips.ethereum.org/EIPS/eip-4337) 通过独立的 `UserOperation`、Bundler 和 EntryPoint 提供不修改共识层的智能账户方案；[**EIP-7702**](https://eips.ethereum.org/EIPS/eip-7702) 则允许 EOA 在交易期间设置可执行委托。二者会并存，钱包、Bundler、Paymaster、恢复人和合约升级权都成为新的安全边界。

### 4.5、零知识证明不等于“匿名币”

零知识证明的核心是：证明者让验证者相信某个命题成立，但不必公开生成证明所用的全部私有输入。

它至少有三类工程用途：

1、扩容：把大量链下计算压缩为可快速验证的证明。

2、隐私：只披露“满足条件”而不披露完整身份或数值。

3、可验证计算：证明某段程序按既定规则执行。

零知识系统仍需关注可信设置、证明系统漏洞、约束电路是否正确、数据可用性和生成证明的成本。使用了 ZK 并不自动意味着整个应用匿名、安全或合规。

### 4.6、跨链桥真正转移的往往不是同一枚资产

跨链常见流程是：源链锁定或销毁，目标链铸造映射资产；或者由流动性提供者在目标链先行兑付。用户看到的是“资产过去了”，底层实际依赖消息验证和资产负债对应关系。

| 桥模型 | 主要信任对象 | 典型风险 |
| --- | --- | --- |
| 多签 / 联盟桥 | 签名人集合 | 密钥泄露、成员串谋 |
| 轻客户端桥 | 对方链共识与验证代码 | 实现漏洞、验证成本 |
| 乐观验证桥 | 观察者和挑战窗口 | 无人挑战、延迟 |
| ZK 验证桥 | 证明系统和验证合约 | 电路、实现或升级权漏洞 |
| 流动性网络 | 流动性提供者和结算机制 | 流动性枯竭、定价与对手方风险 |

因此判断桥的安全性不能只看 TVL 或品牌，还要看验证模型、管理员权限、合约升级延迟、资产托管位置和发生故障时谁能暂停或恢复。

### 4.7、MEV、Sequencer 与“去中心化”之间的现实落差

MEV 是区块生产者、排序者或相关参与者通过改变交易顺序、插入或排除交易获得的额外价值。它会表现为套利、清算，也可能表现为抢跑和三明治攻击。

很多 L2 目前使用单一或少量 Sequencer 提供快速确认。这提升了体验，却带来排序审查、停机和抢先交易风险。评价一条链不能只问“TPS 多高”，还应继续问：

- 谁决定交易顺序？
- 用户能否绕过 Sequencer 直接向 L1 提交？
- 状态根或证明多久提交一次？
- 升级密钥由谁控制，有没有延迟和退出窗口？
- 数据不可用或排序器停机时，用户怎样取回资产？

### 4.8、稳定币与 RWA 正在成为金融基础设施议题

稳定币把链上可转移 Token 与链下储备、银行账户、国债、赎回承诺和发行人治理连接起来。它不是“天然稳定”的算法结果，而是一套资产负债表和法律权利安排。
...
- 谁拥有冻结、拉黑、增发和赎回权限。
- 用户是否有直接向发行人赎回的法律权利。
- 跨链版本由谁发行，桥接资产是否有额外风险。

RWA（Real-World Assets）则把国债、基金份额、存款、应收账款等现实权利数字化。链上 Token 只是技术载体；真正决定权利的是托管、登记、破产隔离、司法辖区和投资者资格。

[**BIS 2025 年年度经济报告第三章**](https://www.bis.org/publ/arpdf/ar2025e3.htm) 把 Tokenization 视为统一账本和下一代货币金融系统的基础设施问题，同时强调稳定币在货币“单一性”、弹性和完整性方面的局限。这比“发一个 Token 就完成资产上链”更接近金融现实。

### 4.9、监管已经从笼统表态进入具体制度

不同司法辖区的监管对象并不相同，不能笼统说“欧洲已经合法化”或“某地允许区块链就等于允许交易所”。

| 地区 | 截至本文更新时间的关键边界 | 阅读方式 |
| --- | --- | --- |
| 中国内地 | [**2021 年通知**](https://www.pbc.gov.cn/tiaofasi/144941/3581332/4348658/index.html) 将虚拟货币相关业务活动定性为非法金融活动；区块链技术研发不等于允许虚拟货币交易经营 | 把技术、资产和金融业务分开判断 |
| 欧盟 | [**MiCA**](https://eur-lex.europa.eu/EN/legal-content/summary/european-crypto-assets-regulation-mica.html) 对加密资产发行人和服务商建立许可、披露与监管框架，主体条款自 `2024-12-30` 起适用 | 受监管不等于无风险，也不等于所有 Token 都有相同法律性质 |
| 美国 | [**GENIUS Act**](https://www.congress.gov/bill/119th-congress/senate-bill/1582) 于 `2025-07-18` 成为法律，建立支付型稳定币发行框架 | 这是稳定币专项框架，不是对所有加密业务的一揽子许可 |
| 中国香港 | [**稳定币发行人监管制度**](https://www.hkma.gov.hk/media/eng/doc/key-functions/ifc/stablecoin-issuers/Guideline_on_supervision_of_licensed_stablecoin_issuers_eng.pdf) 自 `2025-08-01` 生效，由金管局负责许可与监管 | 许可对象、营销限制和储备要求需要逐项核对 |

合规结论必须落到“主体是谁、服务什么客户、提供什么产品、资金和服务器在哪里、是否托管、是否兑换法币”这些具体事实上。本文只作技术和制度地图，不构成法律或投资意见。

### 4.10、后量子密码学要关注，但不是“量子计算一来币就归零”

[**NIST**](https://www.nist.gov/news-events/news/2024/08/announcing-approval-three-federal-information-processing-standards-fips) 在 `2024` 年发布首批三项后量子密码标准 `FIPS 203 / 204 / 205`。区块链目前大量依赖 ECDSA、Schnorr 等经典公钥算法，长期确实需要密码敏捷性和迁移方案。

实际问题包括：

- 哪些公钥已经在链上暴露。
- 新签名的公钥、签名和验证成本是否适合区块空间。
- 老地址怎样迁移，长期不动的资产由谁处理。
- 硬件钱包、合约账户、节点和交易所是否同时支持。
- 协议升级期间如何避免两套密码系统造成新的攻击面。

这是一项协议治理、钱包工程和用户迁移共同参与的长期工作，不只是替换一个算法名字。

### 4.11、当前安全风险更多来自系统边界，而非哈希函数本身

| 风险层 | 常见问题 | 应对思路 |
| --- | --- | --- |
| 用户密钥 | 钓鱼、恶意签名、助记词泄露 | 硬件隔离、交易仿真、限额、撤销授权 |
| 智能合约 | 重入、权限、会计错误、预言机操纵 | 审计、形式化验证、限速、监控与应急暂停 |
| 管理权限 | 单签升级、代理合约被接管 | 多签、时间锁、权限最小化、公开变更窗口 |
| 跨链 | 验证器或消息合约失守 | 明确信任模型、设置额度、隔离桥接资产 |
| L2 | Sequencer 停机、证明或数据延迟 | 强制退出、逃生舱、L1 数据可用性 |
| 业务平台 | 内部账本错误、挤兑、托管失窃 | 储备证明不能替代完整审计，需资产负债与内控共同验证 |

“链本身没被攻破”不代表用户资产安全。大多数真实损失发生在钱包交互、应用合约、桥、预言机、管理员密钥和中心化托管层。

### 4.12、Bitcoin 的发展是保守演进，不是“停止更新”

Bitcoin 基础层优先保持共识稳定，把高频支付、资产协议和复杂应用放到其它层或独立协议中。保守不等于没有工程演进：节点软件仍在持续改进 P2P、钱包、交易中继、费用估算、测试和安全性。

- [**Bitcoin Core 31.0**](https://bitcoincore.org/en/2026/04/19/release-31.0/) 于 `2026-04-19` 发布；节点运营者应同时参考官方[**版本生命周期**](https://bitcoincore.org/en/lifecycle/)，不要把“最新版”理解成所有生产节点必须立即升级。
- Taproot、Schnorr 签名与脚本树属于 Bitcoin 基础协议能力；多签聚合、钱包策略和上层资产协议是否使用它们，是另一个实现层问题。
- Lightning Network 是链下支付通道网络，最终结算依赖 Bitcoin，但它有自己的节点、流动性、路由、在线性和通道管理风险。
- Taproot Assets、Ordinals / Inscriptions、Runes 等属于在 Bitcoin 之上解释或承载资产与数据的协议 / 生态实践；它们不是新的 BTC，也不会修改 `2100 万`的 BTC 供应上限。
- Bitcoin L2、侧链和托管式“BTC 收益产品”安全模型差异很大。看到“由 Bitcoin 保护”时，继续追问资产由谁托管、退出交易是否预签、故障时能否单方面回到 L1，以及是否存在联盟、多签或跨链桥。

学习 Bitcoin 时应同时区分：共识规则、Bitcoin Core 实现、钱包策略、P2P 中继、Lightning / 侧链等上层系统和交易平台托管。把所有名称都写成“Bitcoin 链的新功能”会混淆安全边界。

### 4.13、把当前领域整理成一条可学习路线

1、密码学基础：哈希、签名、密钥派生、Merkle Tree；先纠正“私钥加密、公钥解密”的误区。

2、账本模型：Bitcoin UTXO 与 Ethereum Account Model，理解交易有效性和状态转换。

3、共识与最终性：分别学习 PoW、PoS、分叉选择和经济最终性，不背“51% 节点投票”的口号。

4、智能合约：EVM、Gas、事件、代理升级、权限与常见漏洞。

5、扩容：L1、侧链、Optimistic Rollup、ZK Rollup、Validium 和数据可用性。

6、钱包：HD Wallet、硬件隔离、MPC、多签、账户抽象与 Passkey。

7、应用：稳定币、DEX、借贷、NFT、RWA；每个应用都同时画出技术流、资产流和权利流。

8、安全：从合约审计扩大到密钥、前端、预言机、桥、Sequencer 和治理。

9、合规：按司法辖区区分技术、Token、托管、兑换、支付、证券和衍生品。

10、持续跟踪：只把已上线主网、已生效法律写成事实；路线图、提案和草案单独标注。

### 4.14、2026 年继续跟踪的官方入口

- [**Ethereum Roadmap**](https://ethereum.org/roadmap/)
- [**Ethereum Scaling 文档**](https://ethereum.org/developers/docs/scaling/)
- [**Bitcoin Core 生命周期**](https://bitcoincore.org/en/lifecycle/)
- [**Bitcoin Core 支持的 BIP**](https://github.com/bitcoin/bitcoin/blob/master/doc/bips.md)
- [**NIST Post-Quantum Cryptography**](https://csrc.nist.gov/projects/post-quantum-cryptography/)
- [**FATF Virtual Assets**](https://www.fatf-gafi.org/en/topics/virtual-assets.html)
- [**欧盟 MiCA**](https://eur-lex.europa.eu/EN/legal-content/summary/european-crypto-assets-regulation-mica.html)
- [**中国人民银行：防范和处置虚拟货币交易炒作风险的通知**](https://www.pbc.gov.cn/tiaofasi/144941/3581332/4348658/index.html)

---

## 5、FAQ <a href="#前言摘要" style="font-size:17px; color:green;"><b>🔼</b></a>

* 如果钱包意外收到来源可疑的资产，是否意味着全部资产都“脏了”❓

  “黑 U”不是链协议里的正式状态，而是平台、执法或链上分析服务根据风险线索做出的标签。不同机构采用的归因、聚类和风险阈值不同，因此不能简单断言“一个地址收到一笔，整个助记词下所有资产自动变黑”，也不能保证其它资产一定不受影响。

  合规处理顺序：

  1、不要转移、打散、跨链或与其他自有地址混合，避免破坏证据并扩大关联范围。

  2、保存 TXID、地址、时间、金额、聊天记录、订单和对手方信息，不要只留截图。

  3、联系托管平台、钱包服务商或合规部门，使用正式工单说明“非主动收取”及资金背景。

  4、金额较大、已被冻结或涉及诈骗时，尽快咨询所在地律师并向有管辖权的执法机关报案。

  5、公开浏览器标签只能作为线索，不是司法结论；不要付费给声称可以“解除标签”或“洗白”的个人。

* “2 个风险 U 换 1 个正常 U”的说法可信吗❓

  这通常意味着对方试图转移洗钱、诈骗赃款或制裁规避风险，也可能本身就是二次诈骗。地址跳转、跨链、混币、冷钱包和链下平台都不会把历史事实从链上抹除；分析机构还会结合时间、金额、共同控制和出入金记录重新建立关联。

  本文不提供规避追踪或资产“洗白”方法。正确做法是停止流转、保存证据并走平台申诉、律师和执法渠道。任何要求先交“保证金、解冻费、清洗费”的服务都应按高风险诈骗对待。

* 冷钱包可以改变资金来源记录吗❓

  不可以。冷钱包只负责在隔离环境中保管密钥或签名；接收地址、转账路径、金额和时间仍记录在链上。把资产转入新冷钱包既不会删除历史，也可能把新地址与原地址建立直接关联。
...
* 随着交易的越来越多，那么这个账单就会越来越长，然后撑爆❓
  ...
  * ✅ **不会撑爆用户钱包**
    ...
    ✅ **节点通过不同模式管理存储成本，但容量压力是真实存在的**
  
    - 节点运行时，会维护全链的历史数据，但：
      - 全节点（Full Node）：保存完整区块和交易；
      - 轻节点（Light Node）：只保存区块头，动态查询；
    - 节点可以使用 **快照（snapshot）**、**Pruning（修剪）** 等机制降低本地历史状态开销；归档节点反而会保存更多历史状态，不能把 Archive 当成压缩手段；
    - 链上数据、状态和索引会持续增长，具体容量取决于客户端、同步模式、保留策略和网络阶段，不能用固定“平均区块大小 × 年数”代表全部节点成本；
    - 数据不会凭空消失：某个节点选择修剪，意味着历史数据可用性要由其它节点、归档服务或协议层机制继续承担。
...
<a id="🔚" href="#前言摘要" style="font-size:17px; color:green; font-weight:bold;">我是有底线的➤点我回到首页</a>
